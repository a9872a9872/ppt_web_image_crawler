...
parser.add_argument("--label", "-l", help='選擇 PTT 看板 e.g. Food', default='Food')
parser.add_argument("--pages", "-p", help='設定頁數上限，預設為全部', default=False)
...
def get_newest_page():
    html = requests.get(ptt_host + f'/bbs/{args.label}/index.html', cookies=cookies).text
...
def download_image(page, a, i):
    filename = f'./img/{args.label}/{page}/{i}.jpg'
...
def crawling(page):
    titles = get_html_soup(ptt_host + f'/bbs/{args.label}/index{page}.html').find_all(class_='title')
    i = 1
    for title in titles:
        try:
